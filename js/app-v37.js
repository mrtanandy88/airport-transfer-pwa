...
function startCloudListeners() {
  ...
  if (currentRole === 'customer') {
    ...
    const quoteQ = F.query(F.collection(db, 'customerQuotes'), F.where('customerUid', '==', currentUser.uid));
    stopCloudListeners.push(F.onSnapshot(quoteQ, snap => { customerQuotes = Object.fromEntries(snap.docs.map(d => [d.id, d.data()])); render(); }, handleCloudError));
  }
  if (currentRole === 'driver') {
...
async function logout() { if (firebaseReady && auth) await window.FB.signOut(auth); stopListeners(); currentUser = null; currentRole = null; driverProfile = null; driverPublicProfile = null; bookings = []; schedules = []; driverAvailable = []; driverAccepted = []; customerQuotes = []; updateAuthUI(); render(); }
...
function driverJobFeeHtml(b){
  ...
  return '<div class="driver-job-fee">💰 <b>Driver job fee: '+escapeHtml(formatJobFee(fee?.fee))+'</b></div>';
}
function customerQuoteHtml(b){
  const quote=customerQuotes[b.docId];
  const price=quote && Number.isFinite(Number(quote.price)) ? Number(quote.price) : null;
  if(price===null){
    return '<div class="customer-quote pending">💬 <b>Customer price: Quote pending</b><small>Admin will confirm your transparent price.</small></div>';
  }
  return '<div class="customer-quote confirmed">💳 <span>Customer price</span><strong>RM '+price.toFixed(2)+'</strong><small>Transparent customer price • Please confirm with admin on WhatsApp before payment.</small></div>';
}
...
function adminCustomerQuoteHtml(b){
  const quote=customerQuotes[b.docId];
  const current=quote && Number.isFinite(Number(quote.price)) ? 'Current: RM '+Number(quote.price).toFixed(2) : 'Not set';
  return '<div class="admin-customer-quote"><b>Customer price:</b> '+escapeHtml(current)+' <button class="secondary" type="button" data-quote-id="'+escapeHtml(b.docId)+'" onclick="setCustomerQuote(this.dataset.quoteId)">Set / change quote</button></div>';
}
function renderJobs(){
  if(currentRole!=='driver'){
...
function renderCustomerBookings(skipRefresh=false){
  ...
  $('#customerBookings').innerHTML='<div class="customer-bookings-header"><div><h3>Your bookings</h3><p class="muted">Your existing bookings are shown below.</p></div><button class="secondary" type="button" onclick="startNewBooking()">＋ New booking</button></div>'+list.map(b=>`<div class="booking"><h3>${escapeHtml(b.id)} <span class="badge">${escapeHtml(b.status)}</span></h3><p>${escapeHtml(b.pickup)} → ${escapeHtml(b.destination)}</p><div class="trip-schedule compact"><span>🗓 TRIP SCHEDULE</span><strong>${escapeHtml(formatDate(b.date))}</strong><b>${escapeHtml(formatTime(b.time))}</b></div>${locationLinks(b,false)}<div class="customer-party-summary"><div>👤 <b>${escapeHtml(String(b.adults ?? b.passengers ?? 0))}</b> adult${Number(b.adults ?? b.passengers ?? 0)===1?'':'s'}${Number(b.children ?? 0)>0?' • '+escapeHtml(String(b.children))+' children':''}</div><div>🧳 <b>${escapeHtml(String(b.luggage ?? 0))}</b> total bags • Checked: ${escapeHtml(String(b.checkedLuggage ?? b.luggage ?? 0))} • Hand carry: ${escapeHtml(String(b.handCarry ?? 0))}</div><div class="luggage-breakdown">Large: ${escapeHtml(String(b.largeLuggage ?? 0))} • Medium: ${escapeHtml(String(b.mediumLuggage ?? 0))} • Small: ${escapeHtml(String(b.smallLuggage ?? 0))}</div></div><p>${escapeHtml(b.language)} • Vehicle: <b>${escapeHtml(b.vehicleType||'Not specified')}</b></p>${customerQuoteHtml(b)}${b.flightNumber||b.flightType||b.terminal||b.meetInstructions?`<div class="flight-details"><b>✈️ FLIGHT DETAILS</b><br>${b.flightType?`Trip: ${escapeHtml(b.flightType)}<br>`:''}${b.flightNumber?`Flight: <b>${escapeHtml(b.flightNumber)}</b><br>`:''}${b.flightDate||b.flightTime?`Flight schedule: ${escapeHtml(formatDateTime(b.flightDate,b.flightTime))}<br>`:''}${b.terminal?`Terminal: ${escapeHtml(b.terminal)}<br>`:''}${b.meetInstructions?`Meet / pickup: ${escapeHtml(b.meetInstructions)}`:''}</div>`:''}${b.status==='Accepted'?renderTripStatus(b,false)+driverCard(b,publicProfileCache.get(b.driverUid))+`<div class="whatsapp-actions"><a class="whatsapp-button" href="${adminWhatsAppLink(b,b.status==='Available'?'newBooking':'support')}" target="_blank" rel="noopener">💬 WhatsApp admin</a><a class="whatsapp-button secondary-whatsapp" href="${adminWhatsAppLink(b,'group')}" target="_blank" rel="noopener">👥 Prepare WhatsApp group</a></div>`:`<div class="booking-waiting"><small>Waiting for a matched driver</small>${customerQuoteHtml(b)}<div class="whatsapp-actions"><a class="whatsapp-button" href="${adminWhatsAppLink(b,'support')}" target="_blank" rel="noopener">💬 Contact admin on WhatsApp</a></div></div>`}</div>`).join('');
...
window.setCustomerQuote=async id=>{
  if(!firebaseReady||currentRole!=='admin') return alert('Admin access required.');
  const booking=bookings.find(x=>x.docId===id||x.id===id);
  if(!booking) return;
  const existing=customerQuotes[id]?.price;
  const raw=prompt('Enter the transparent customer price in RM for '+(booking.id||id), existing!=null?String(existing):'');
  if(raw===null) return;
  const price=Number(String(raw).replace(/,/g,'').trim());
  if(!Number.isFinite(price)||price<0||price>100000) return alert('Please enter a valid customer price from RM 0 to RM 100,000.');
  try{
    await window.FB.setDoc(window.FB.doc(db,'customerQuotes',id),{
      bookingId:id,
      bookingCode:booking.id||'',
    ...
    alert('Customer price saved: RM '+price.toFixed(2));
  }catch(e){ alert('Could not save customer price ('+(e.code||'error')+'): '+e.message); }
};
...
function buildHomeQuoteMessage(){
  const pickup=$('#homeQuotePickup')?.value.trim()||'';
  const destination=$('#homeQuoteDestination')?.value.trim()||'';
  const date=$('#homeQuoteDate')?.value||'';
  const time=$('#homeQuoteTime')?.value||'';
  const vehicle=$('#homeQuoteVehicle')?.value||'';
  const passengers=$('#homeQuotePassengers')?.value||'';
  const luggage=$('#homeQuoteLuggage')?.value||'';
  const name=$('#homeQuoteName')?.value.trim()||'';
  const phone=$('#homeQuotePhone')?.value.trim()||'';
  if(!pickup||!destination||!date||!time||!vehicle||!passengers||!name||!phone){
    alert('Please complete your name, WhatsApp number, pickup, destination, date, time, vehicle and passengers.');
    return null;
  }
  return [
    'Airport Transfer Price Quote Request',
    '',
    'Customer: '+name,
    'WhatsApp: '+phone,
    'Pickup: '+pickup,
    'Destination: '+destination,
    'Trip: '+formatDateTime(date,time),
    'Vehicle: '+vehicle,
    'Passengers: '+passengers,
    'Luggage: '+(luggage||'0'),
    '',
    'Please provide the transparent customer price for this airport transfer.'
  ].join('\n');
}
$('#homeQuoteWhatsApp')?.addEventListener('click',()=>{
  const message=buildHomeQuoteMessage();
  if(message) window.open(whatsappUrl(ADMIN_WHATSAPP,message),'_blank','noopener');
});
