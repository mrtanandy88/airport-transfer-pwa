...
let bookings=JSON.parse(localStorage.getItem(KEY)||'[]'), schedules=JSON.parse(localStorage.getItem(SKEY)||'[]');
...
async function initFirebase(){
 ...
 try{
  ...
  auth=am.getAuth(fb); db=fs.getFirestore(fb); window.FB={...am,...fs}; firebaseReady=true;
  $('#modeNotice').textContent='Firebase connected. Sign in to sync bookings across devices.';
  am.onAuthStateChanged(auth,async u=>{
   currentUser=u; currentRole=null;
   if(u){
    const snap=await window.FB.getDoc(window.FB.doc(db,'users',u.uid));
    currentRole=snap.exists()?snap.data().role:null;
    if(currentRole)localStorage.setItem('atRole',currentRole); else localStorage.removeItem('atRole');
    if(currentRole==='driver')await loadCloudSchedules();
    if(currentRole==='admin')await loadCloudBookings();
    if(currentRole==='customer')await loadCloudBookingsForCustomer();
   }else{localStorage.removeItem('atRole');bookings=[];schedules=[];}
   updateAuthUI(); render();
  });
 }catch(e){console.error(e);firebaseReady=false;$('#modeNotice').textContent='Firebase could not start; demo mode is active.';}
}
...
function updateAuthUI(){
 ...
 $('#customerAuth').classList.toggle('hidden',firebaseReady&&signed&&currentRole==='customer');
 $('#bookingForm').classList.toggle('hidden',firebaseReady&&(!signed||currentRole!=='customer'));
...
async function signUp(role,email,password){
 ...
 try{
  const r=await window.FB.createUserWithEmailAndPassword(auth,email,password);
  await window.FB.setDoc(window.FB.doc(db,'users',r.user.uid),{email,role,createdAt:window.FB.serverTimestamp()});
...
async function signIn(role,email,password){
 ...
 if(!email||!password)return alert('Enter email and password.');
 try{
  const r=await window.FB.signInWithEmailAndPassword(auth,email,password);
  const snap=await window.FB.getDoc(window.FB.doc(db,'users',r.user.uid));
  const actualRole=snap.exists()?snap.data().role:null;
  if(actualRole!==role){await window.FB.signOut(auth);return alert(`This account is registered as ${actualRole||'unknown'}, not ${role}.`);}
 }catch(e){alert(e.message);}
}
async function logout(){if(firebaseReady&&auth)await window.FB.signOut(auth);currentUser=null;currentRole=null;updateAuthUI();render();}
...
async function addBooking(b){
 if(firebaseReady){
  await window.FB.addDoc(window.FB.collection(db,'bookings'),{...b,customerUid:currentUser.uid,createdAt:window.FB.serverTimestamp()});
  await loadCloudBookingsForCustomer();
 }else{bookings.unshift(b);saveLocal();}
}
$('#bookingForm').addEventListener('submit',async e=>{
 e.preventDefault();const f=new FormData(e.target),b={id:'AT-'+Date.now().toString().slice(-6),name:f.get('name'),phone:f.get('phone'),pickup:f.get('pickup'),destination:f.get('destination'),date:f.get('date'),time:f.get('time'),passengers:Number(f.get('passengers')),luggage:Number(f.get('luggage')||0),language:f.get('language'),status:'Available',driver:'',driverUid:''};
 try{await addBooking(b);e.target.reset();$('#customerResult').innerHTML=`<div class="booking success"><h3>Booking placed ✓</h3><p>Your booking ID is <b>${b.id}</b>.</p><span class="badge">Available</span></div>`;}catch(err){alert(err.message);}
});
...
function overlapsSchedule(b,s){return !!s&&s.date===b.date&&String(b.time)>=String(s.from)&&String(b.time)<=String(s.to)}
function canDriverTake(b){return !schedules.some(s=>overlapsSchedule(b,s));}
async function acceptJob(id){
 const b=bookings.find(x=>(x.docId||x.id)===id);if(!b)return;if(!canDriverTake(b))return alert('You have an unavailable schedule overlapping this booking.');
 if(firebaseReady){
  await window.FB.updateDoc(window.FB.doc(db,'bookings',b.docId),{status:'Accepted',driver:currentUser.email,driverUid:currentUser.uid,acceptedAt:window.FB.serverTimestamp()});
  await loadCloudBookings();
 }else{b.status='Accepted';b.driver='Demo Driver';saveLocal();}
}window.acceptJob=acceptJob;
async function addUnavailable(){const date=$('#unavailableDate').value,from=$('#unavailableFrom').value,to=$('#unavailableTo').value;if(!date||!from||!to||from>=to)return alert('Please enter a valid unavailable date and time.');const s={date,from,to,driverUid:currentUser?.uid||'demo'};if(firebaseReady){await window.FB.addDoc(window.FB.collection(db,'driverSchedules'),s);await loadCloudSchedules();}else{s.id='S-'+Date.now();schedules.push(s);saveLocal();}}$('#addUnavailable').onclick=addUnavailable;
function renderSchedules(){$('#scheduleList').innerHTML=schedules.length?schedules.map(s=>`<div class="schedule"><b>${s.date}</b><span>${s.from}–${s.to}</span></div>`).join(''):'<p class="muted">No unavailable times added.</p>'}
function render(){
 renderSchedules();const available=bookings.filter(b=>b.status==='Available'&&canDriverTake(b));
