...
function renderJobs(){ if(currentRole!=='driver'){ $('#driverJobs').innerHTML=''; return; } const available=driverAvailable.filter(b=>b.status==='Available'&&canDriverTake(b)); if(!available.length){const vehicle=driverProfile?.vehicleType; $('#driverJobs').innerHTML=vehicle?'<p class="muted">No jobs currently matching your vehicle, language, private schedule and existing jobs.</p>':'<p class="muted">Complete your driver profile to see matching jobs.</p>';return;} $('#driverJobs').innerHTML=available.map(b=>`<div class="booking driver-job-card"><div class="job-route"><h3>${escapeHtml(b.pickup)} → ${escapeHtml(b.destination)}</h3></div><div class="trip-schedule"><span>🗓 TRIP SCHEDULE</span><strong>${escapeHtml(formatDate(b.date))}</strong><b>${escapeHtml(formatTime(b.time))}</b></div><p>${escapeHtml(String(b.passengers))} passenger(s) • ${escapeHtml(String(b.luggage ?? 0))} luggage • Language: <b>${escapeHtml(b.language)}</b></p>${b.flightNumber||b.flightType||b.terminal||b.meetInstructions?`<div class="flight-details"><b>✈️ FLIGHT DETAILS</b><br>${b.flightType?`Trip: ${escapeHtml(b.flightType)}<br>`:''}${b.flightNumber?`Flight: <b>${escapeHtml(b.flightNumber)}</b><br>`:''}${b.flightDate||b.flightTime?`Flight schedule: ${escapeHtml(formatDateTime(b.flightDate,b.flightTime))}<br>`:''}${b.terminal?`Terminal: ${escapeHtml(b.terminal)}<br>`:''}${b.meetInstructions?`Meet / pickup: ${escapeHtml(b.meetInstructions)}`:''}</div>`:''}<p>Vehicle: <b>${escapeHtml(b.vehicleType||'Not specified')}</b></p><small>${escapeHtml(b.name)} • ${escapeHtml(b.id)} • ${escapeHtml(matchReasons(b).join(' • '))}</small><button class="accept" onclick="acceptJob('${escapeHtml(b.docId)}')">Accept job</button></div>`).join(''); }
