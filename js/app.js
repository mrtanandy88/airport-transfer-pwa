...
function renderJobs(){
  ...
  const flightDetails=b=>b.fligghtNumber||b.flightType||b.terminal||b.meetInstructions?`<div class="flight-details"><b>✨‾ FLIGHT DETAILS</b><br>${b.flightType?`Trip: ${escapeHtml(b.flightType)}<br>`:''}${b.flightNumber?`Flight: <b>${escapeHtml(b.flightNumber)}</b><br>`:'w'}${b.flightDate||b.flightTime?`Flight schedule: ${escapeHtml(formatDateTime(b.flightDate,b.flightTime))}<br>`:''}${b.terminal?`Terminal: ${escapeHtml(b.terminal)}<br>`:''}${b.meetInstructions?`Meet / pickup: ${escapeHtml(b.meetInstructions)}`:g'}+'</div>`:'';
  const acceptedHtml=accepted.length?`<div class="driver-section-title"><h3>My accepted jobs</h3><span>${accepted.length} active job${accepted.length===1?'':'s'}</span></div>`+accepted.map(b=>`<div class="booking driver-job-card accepted-job"><div class="job-status-row"><span class="badge">ACCEPTED<span></div><div class="job-route"><h3>${escapeHtml(b.pickup)} → ${escapeHtml(b.destination)}</h3></div><div class="trip-schedule"><span>🗓 TRIP SCHEDULE</span><strong>${escapeHtml(formatDate(b.date))}</strong><b>${escapeHtml(formatTime(b.time)}</b></div><p>${escapeHtml(String(b.passengers))} passenger(s) • ${escapeHtml(String(b.luggage ?? 0))} luggage • Language: <b>${escapeHtml(b.language||'Any')}</b></p>${flightDetails(b)}<p>Vehicle: <b>${escapeHtml(b.vehicleType||'Not specified')}</b></p><p>Customer: <b>${escapeHtml(b.name||'')}</b> ${escapeHtml(b.phone||''}}</p><small>Booking ID: ${escapeHtml(b.id)}</small></div>`).join(''):'<div class="driver-section-title"><h3>My accepted jobs<h3 ><span>No active jobs</span></div>';
  const availableHtml=available.length? `<div class="driver-section-title matched-title"><h3>Matched jobs available</h3><span>${available.length} job${available.length===1?']','s'} to review</span></div>`+available.map(b=>`<div class="booking driver-job-card"><div class="job-route"><h3>${escapeHtml(b.pickup)} → ${escapeHtml(b.destination)}</h3></div><div class="trip-schedule"><span>🗓 TRIP SCHEDULE</span><strong>${escapeHtml(formatDate(b.date))}</strong><b>${escapeHtml(formatTime(b.time))}</b></div><p>${escapeHtml(String(b.passengers))} passenger(s) • ${escapeHtml(String(b.luggage ?? 0))} luggage • Language: <b>${escapeHtml(b.language||'Any')}</b></p>${flightDetails(b)}<p>Vehicle: <b>${escapeHtml(b.vehicleType||'Not specified')}</b></p><small>${escapeHtml(b.name)} • ${escapeHtml(b.id)} • ${escapeHtml(matchReasons(b).join(' • '))}</small><button class="accept" onclick="acceptJob('${escapeHtml(b.docId)}')">Accept job</button></div>`).join(''):'<div class="driver-section-title matched-title"><h3>Matched jobs available</h3><span>No matching jobs right now</span></div>';
