...
function updateAuthUI() {
  ...
  $('#customerAuth').classList.toggle('hidden', signed && currentRole === 'customer'); $('#bookingForm').classList.toggle('hidden', !(signed && currentRole === 'customer')); $('#customerLogout').classList.toggle('hidden', !(signed && currentRole === 'customer'));
...
$('#bookingForm').addEventListener('submit', async e => {
  ...
  const booking = { id: 'AT-' + Date.now().toString().slice(-6), name: f.get('name'), phone: f.get('phone'), pickup: f.get('pickup'), destination: f.get('destination'), date: f.get('date'), time: f.get('time'), passengers: Number(f.get('passengers')), luggage: Number(f.get('luggage') || 0), flightType: f.get('flightType') || '', flightNumber: String(f.get('flightNumber') || '').trim().toUpperCase(), flightDate: f.get('flightDate') || '', flightTime: f.get('flightTime') || '', terminal: String(f.get('terminal') || '').trim(), meetInstructions: String(f.get('meetInstructions') || '').trim(), language: f.get('language'), vehicleType, status: 'Available', driver: '', driverUid: '' };
...
function renderSchedules(){ $('#scheduleList').innerHTML=schedules.length?schedules.sort((a,b)=>`${a.date}${a.from}`.localeCompare(`${b.date}${b.from}`)).map(s=>`<div class="schedule"><b>${escapeHtml(s.date)}</b><span>${escapeHtml(s.from)}–${escapeHtml(s.to)} <button class="mini-delete" onclick="deleteSchedule('${escapeHtml(s.docId)}')" type="button">×</button></span></div>`).join(''):'<p class="muted">No unavailable times added.</p>'; }
function renderJobs(){ if(currentRole!=='driver'){ $('#driverJobs').innerHTML=''; return; } const available=driverAvailable.filter(b=>b.status==='Available'&&canDriverTake(b)); if(!available.length){const vehicle=driverProfile?.vehicleType; $('#driverJobs').innerHTML=vehicle?'<p class="muted">No jobs currently matching your vehicle, language, private schedule and existing jobs.</p>':'<p class="muted">Complete your driver profile to see matching jobs.</p>';return;} $('#driverJobs').innerHTML=available.map(b=>`<div class="booking"><h3>${escapeHtml(b.pickup)} → ${escapeHtml(b.destination)}</h3><p>${escapeHtml(b.date)} at ${escapeHtml(b.time)} • ${escapeHtml(String(b.passengers))} passenger(s) • ${escapeHtml(String(b.luggage ?? 0))} luggage • ${escapeHtml(b.language)}</p>${b.flightNumber||b.flightType||b.terminal||b.meetInstructions?`<div class="flight-details"><b>✈️ Flight details</b><br>${b.flightType?`Trip: ${escapeHtml(b.flightType)}<br>`:''}${b.flightNumber?`Flight: <b>${escapeHtml(b.flightNumber)}</b><br>`:''}${b.flightDate||b.flightTime?`Flight schedule: ${escapeHtml(b.flightDate||'')} ${escapeHtml(b.flightTime||'')}<br>`:''}${b.terminal?`Terminal: ${escapeHtml(b.terminal)}<br>`:''}${b.meetInstructions?`Meet / pickup: ${escapeHtml(b.meetInstructions)}`:''}</div>`:''}<p>Vehicle: <b>${escapeHtml(b.vehicleType||'Not specified')}</b></p><small>${escapeHtml(b.name)} • ${escapeHtml(b.id)} • ${escapeHtml(matchReasons(b).join(' • '))}</small><button class="accept" onclick="acceptJob('${escapeHtml(b.docId)}')">Accept job</button></div>`).join(''); }
...
function renderCustomerBookings(skipRefresh=false){ if(currentRole!=='customer'){ $('#customerBookings').innerHTML=''; return; } const list=[...bookings].sort((a,b)=>String(b.date+b.time).localeCompare(String(a.date+a.time))); $('#customerBookings').innerHTML=list.length?`<h3>Your bookings</h3>`+list.map(b=>`<div class="booking"><h3>${escapeHtml(b.id)} <span class="badge">${escapeHtml(b.status)}</span></h3><p>${escapeHtml(b.pickup)} → ${escapeHtml(b.destination)}</p><p>${escapeHtml(b.date)} ${escapeHtml(b.time)} • ${escapeHtml(b.language)} • Vehicle: <b>${escapeHtml(b.vehicleType||'Not specified')}</b></p>${b.flightNumber||b.flightType||b.terminal||b.meetInstructions?`<div class="flight-details"><b>✈️ Flight details</b><br>${b.flightType?`Trip: ${escapeHtml(b.flightType)}<br>`:''}${b.flightNumber?`Flight: <b>${escapeHtml(b.flightNumber)}</b><br>`:''}${b.flightDate||b.flightTime?`Flight schedule: ${escapeHtml(b.flightDate||'')} ${escapeHtml(b.flightTime||'')}<br>`:''}${b.terminal?`Terminal: ${escapeHtml(b.terminal)}<br>`:''}${b.meetInstructions?`Meet / pickup: ${escapeHtml(b.meetInstructions)}`:''}</div>`:''}<p>Luggage: <b>${escapeHtml(String(b.luggage ?? 0))}</b></p>${b.status==='Accepted'?driverCard(b,publicProfileCache.get(b.driverUid)): '<small>Waiting for a matched driver</small>'}</div>`).join(''):'<p class="muted">No bookings yet.</p>'; if(!skipRefresh&&list.some(b=>b.driverUid))refreshCustomerDriverProfiles(); }
function render(){ renderSchedules(); renderJobs(); renderCustomerBookings(); const total=bookings.length,availableCount=bookings.filter(b=>b.status==='Available').length,accepted=bookings.filter(b=>b.status==='Accepted').length,completed=bookings.filter(b=>b.status==='Completed').length; $('#adminStats').innerHTML=`<div class="stat"><b>${total}</b><small>Total</small></div><div class="stat"><b>${availableCount}</b><small>Available</small></div><div class="stat"><b>${accepted}</b><small>Accepted</small></div><div class="stat"><b>${completed}</b><small>Completed</small></div>`; const list=filter==='All'?bookings:bookings.filter(b=>b.status===filter); $('#adminBookings').innerHTML=list.length?list.map(b=>`<div class="booking"><h3>${escapeHtml(b.id)} <span class="badge">${escapeHtml(b.status)}</span></h3><p><b>${escapeHtml(b.pickup)}</b> → ${escapeHtml(b.destination)}</p><p>${escapeHtml(b.date)} ${escapeHtml(b.time)} • ${escapeHtml(b.name)} • ${escapeHtml(b.phone)}</p><p>Vehicle: <b>${escapeHtml(b.vehicleType||'Not specified')}</b> • Language: ${escapeHtml(b.language||'Any')} • Luggage: <b>${escapeHtml(String(b.luggage ?? 0))}</b></p>${b.flightNumber||b.flightType||b.terminal||b.meetInstructions?`<div class="flight-details"><b>✈️ Flight details</b><br>${b.flightType?`Trip: ${escapeHtml(b.flightType)}<br>`:''}${b.flightNumber?`Flight: <b>${escapeHtml(b.flightNumber)}</b><br>`:''}${b.flightDate||b.flightTime?`Flight schedule: ${escapeHtml(b.flightDate||'')} ${escapeHtml(b.flightTime||'')}<br>`:''}${b.terminal?`Terminal: ${escapeHtml(b.terminal)}<br>`:''}${b.meetInstructions?`Meet / pickup: ${escapeHtml(b.meetInstructions)}`:''}</div>`:''}<small>${b.driver?`Driver: ${escapeHtml(b.driver)} • ${escapeHtml(b.driverCarModel||'')} • ${escapeHtml(b.driverCarColor||'')} • Plate: ${escapeHtml(b.driverPlateNumber||'')}`:'No driver yet'}</small>${b.status==='Accepted'&&currentRole==='admin'?`<button class="secondary complete" onclick="completeJob('${escapeHtml(b.docId)}')">Mark completed</button>`:''}</div>`).join(''):'<p class="muted">No bookings.</p>'; }
